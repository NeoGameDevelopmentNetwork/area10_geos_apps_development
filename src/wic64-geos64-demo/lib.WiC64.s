...
; v0.40: Updated.
;        _WiC64_Handshake:
;        TimeOut-Zähler angepasst.
;
;        _WiC64_SendCom:
;        Verzögerung nach dem senden
;        des Datenbyte ergänzt (SCPU).
;
;        _WiC64_GetSSID:
;        Länge der SSID auf 40 Zeichen
;        erweitert (VICE/SSID).
;
; v0.30: Updated.
;        _WiC64_CHECK:
;        Test auf SCPU/TC64 ergänzt.
;
;        _WiC64_Handshake:
;        Zusätzliche Verzögerung/NOP vor dem abrufen
;        des Datenbyte ergänzt.
;        Warteschleife von Timer auf Raster-IRQ geändert und
;        TimeOut-Zähler angepasst.
;
; v0.20: Updated.
;        _WiC64_InitGet2:
;        Dateilängenprüfung optimiert.
;
...
;*** Auf WiC64 testen.
:_WiC64_CHECK		jsr	_WiC64_HW_TC64		;TurboChameleon64 erkennen.
...
;ACHTUNG: Reverse Order High/Low!
			...
			ora	lenDataSize +2
			ora	lenDataSize +1		;Datengröße testen.
::done			ora	lenDataSize +0		;Größer > 0 Bytes?
...
::skip			ldx	r11L			;Anzahl Daenbytes -1.
			bne	:21
...
;Verändert: A
;
:_WiC64_Wait		bit	flagWiC64dbg		;Debug-Modus?
			bpl	:do_wait		; => Nein, weiter...

			inc	extclr			;Rahmenfarbe wechseln für

::do_wait		nop				;Minimale Verzögerung.
			nop
			nop
			nop

;--- Warteschleife über TOD:
;			lda	CIA_TOD10		;Bei Turbo-Karten 1/10sec. warten.
;::wait			cmp	CIA_TOD10		;(dec/bne läuft hier zu schnell)
;			beq	:wait

;--- Warteschleife über Rasterzeile:
...
if WiC64DebugMode = FALSE
:flagWiC64dbg		b $00 ;Bit%7=0: Debug-Modus inaktiv.
else
:flagWiC64dbg		b $80 ;Bit%7=1: Debug-Modus aktiv.
endif
...
if ENABLE_GETSSID = TRUE
:max_getnam_data	= 40 ;Freie Begrenzung auf 40 Zeichen (Bildschirmausgabe).
...
if ENABLE_GETTZN = TRUE
:max_gettzn_data	= 8  ;Freie Begrenzung auf 8 Zeichen.
