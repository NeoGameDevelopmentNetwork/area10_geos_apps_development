...
:vPutBlock_dskBuf	jmp	PutBlock_dskBuf
			nop				;1541: TurboRoutine_r1
			nop
			rts
			nop				;1541: GetDiskError
			nop
			rts
...
:xGetDirHead		jsr	SetBAM_TrSe1		;Zeiger auf BAM-Sektor/-Speicher.
			...
			bne	xGetBlock		;BAM-Sektor einlesen/konvertieren.
::51			rts

;*** Sektor nach ":diskBlkBuf" einlesen.
:GetBlock_dskBuf	lda	#> diskBlkBuf		;Zeiger auf ":diskBlkBuf" setzen.
			sta	r4H
			lda	#< diskBlkBuf
			sta	r4L
...
:PutBlock_dskBuf	lda	#> diskBlkBuf		;Zeiger auf ":diskBlkBuf" setzen.
			sta	r4H
			lda	#< diskBlkBuf
			sta	r4L
...
:xOpenDisk		jsr	NewDisk			;Neue Diskette initialisieren.
			txa				;Diskettenfehler ?
			bne	:51			;Ja, Abbruch...
...
::51			jsr	Set_curDirHead		;Zeiger auf aktuelle BAM.
			jsr	CalcBlksFree		;Anzahl freie Sektoren berechnen.
...
:TestNextSetFree	lda	r3H			;Zeiger auf nächsten Sektor.
			clc
			adc	#$01
			sta	r6H
...
::55			beq	:57			; => Track belegt, weitersuchen.
			lda	#$28			;Max. Anzahl Sektoren auf Track
...
;*** Sektor aus Partition einlesen.
:ReadSekRAM		ldy	#$91
			bne	GetDataBytes

;*** Sektor in Partition kopieren.
:WriteSekRAM		ldy	#$90
			bne	GetDataBytes
...
;*** CopyRight-Meldung.
			b "RAM 1581 driver "
			b "Copyright (C) 1990, Jim Collette & "
			b "Berkeley Softworks.  "
			b "Thanks to Matt Loveless and Angie McKenna "
			b "of BSW for their help and support.  "
			b "(Q-Link: GEOREP JIM)"
			b NULL
