...
:xTestNewDisk		jsr	ExitTurbo		;TurboDOS abschalten.
			jsr	InitForIO		;I/O-Bereich einblenden.

			lda	#< com_ME_CRC
			ldx	#> com_ME_CRC
			jsr	SendFCom_CRC1		;Diskwechsel abfragen.
			jsr	UNLSN

			ldx	#> com_MR_CRC
			lda	#< com_MR_CRC
			ldy	#$06
			jsr	SendFCom_CRC2		;Wurde Disk gewechselt?
			jsr	UNLSN
			...
			tay
			lda	#$ff			; => CRC ungültig.
			cpy	LastCRC +0		;Wurde Disk gewechselt?
			bne	:201
			cpx	LastCRC +1
			bne	:201			; => Ja, neuen CRC-Wert speichern.
			lda	#$00			; => CRC gültig, kein Diskwechsel.
::201			sty	LastCRC +0
			stx	LastCRC +1
			tax
...
::55			jmp	DoneWithIO		;I/O-Bereich ausblenden.
