...
:wheelDelay		b WHEEL_DELAY			;Verzögerung für Mausrad.
:curDelay		b $00				;>0 = Verzögerung aktiv.
...
::no_wheel		lda	cia1base +1		;Maustasten-Register einlesen
...
;Bit%0: Rechte Taste.
			and	#%00011111		;Maustasten gedrückt?
...
:TestM3Key		lsr				;Mittlere Maustaste gedrückt?
			bcc	TestWheelUp		; => Nein, weiter...

			ldx	#%00100000		;inputData +1: Mittlere Maustaste.
			bne	ExecMseClick

;--- Mausrad aufwärts abfragen.
:TestWheelUp		lsr				;Mausrad aufwärts?
			bcc	TestWheelDown		; => Nein, weiter...

if WHEEL_KEYS = TRUE
			ldx	#WHEEL_UP		;Cursor up.
			lda	#%00000000		;inputData +1: Wheel ignorieren.
else
			lda	#%00010000		;inputData +1: Wheel up.
endif
			bcs	ExecWheel

;--- Mausrad abwärts abfragen.
:TestWheelDown		lsr				;Mausrad abwärts?
			bcc	DoneWheel		; => Nein, Keine Maustaste gedrückt...

if WHEEL_KEYS = TRUE
			ldx	#WHEEL_DOWN		;Cursor down.
			lda	#%00000000		;inputData +1: Wheel ignorieren.
else
			lda	#%00001000		;inputData +1: Wheel down.
endif
;			bcs	ExecWheel

;--- Mausrad-Bewegung ausführen.
:ExecWheel

;--- Hinweis:
;Die Routine ":PutKeyInBuffer" ist nur
;unter MP3/GDOS64 verfügbar!
if WHEEL_KEYS = TRUE
			ldy	MP3_CODE +0		;GEOS MP3/GDOS64 aktiv?
			cpy	#"M"
			bne	NoMKeyActive
			ldy	MP3_CODE +1
			cpy	#"P"
			bne	NoMKeyActive		; => Nein, weiter...
endif

			ldy	curDelay		;Verzögerung aktiv?
			bne	NoMKeyActive		; => Ja, weiter...

			ldy	wheelDelay		;Verzögerung neu setzen.
			sty	curDelay
...
if WHEEL_MKEY = TRUE
			...
			txa				;Taste ausführen.
::1			pha
			jsr	PutKeyInBuffer		;Taste ausführen.
			pla
...
;--- Mausrad-Aktion ausgeführt.
:DoneWheel		tax				;":inputData" +1: Wheel up/down.
			beq	StartMseMove		; => Kein Mausrad, Status nicht ändern.

							;":mouseData" nicht ändern, da sonst
			bne	SetKeyData		;das Mausrad als Taste erkannt wird.
