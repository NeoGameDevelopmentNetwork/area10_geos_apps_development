...
:BASE_TASKBUF		= $e000				;Adresse TaskManager-Zwischenspeicher/REU.
:SWAP_TASKBUF		= $6000				;Zwischenspeicher TaskManager.
...
;*** Task-Namen.
;ACHTUNG!
;Alle Task-Namen sind 16+NULL Byte groß!
;                   -1234567890123456-
:TaskName00		b "< GDOS-SYSTEM! >"
			e TaskName00 +17
...
:MainInit		sta	AkkuBuf			;Register speichern.
			...
			jmp	BackToCurTask		; => Zum aktuellen Task wechseln.
...
;--- Hinweis:
;Hier aktualisiert GDOS64 die BAM im
...
;IRQ-Rotuinen laufen ggf. ins leere.
;Beispiel MP3-128 => GeoPaint128.
:InitTaskMenu		jsr	GEOS_InitSystem		;GEOS-Variablen zurücksetzen.

			jsr	UpdateMouse		;Maustasten aktualisieren.
...
:PrepareExit		jsr	PurgeTurbo		;TurboDOS entfernen.
...
:SetRAM_TaskSys		lda	#$00			;C64 : $6000
			...
			ldx	#> RTS_TASKMAN
			stx	r2H
;			LoadW	r0,SWAP_TASKBUF
;			LoadW	r1,LOAD_TASKMAN
...
:SetRAM_MP_VAR		lda	#< EXTVAR_BASE		;C64 : varDataSys
			sta	r0L
			sta	r1L
			lda	#> EXTVAR_BASE		;REU : varDataSys
			sta	r0H
			sta	r1H
			lda	#< EXTVAR_SIZE
			sta	r2L
			lda	#> EXTVAR_SIZE
			sta	r2H
;			LoadW	r0,EXTVAR_BASE
;			LoadW	r1,EXTVAR_BASE
;			LoadW	r2,EXTVAR_SIZE
			jmp	SetRAM_TaskBank
...
;    Übergabe:		Akku = Dateityp.
:Slct_ApplExec		bit	Flag_ExtRAMinUse	;Ext. Speicher testen.
			bmi	:51			; => Hilfsmittel aktiv ?
			...
			jsr	Slct_FileType		;Datei auswählen.
...
;			jsr	SetNewTaskName		;Dateiname in Tabelle eintragen.
			clc
			rts
...
;*** Dokument öffnen.
:Slct_ApplData		bit	Flag_ExtRAMinUse	;Ext. Speicher testen.
			bmi	:51			; => Hilfsmittel aktiv ?
			bvs	:51			; => Dialogbox aktiv ?
			...
			jsr	doSelectFile		;Datei auswählen.
			bcs	:51			;Abbruch = => Ja, Ende...
...
:Slct_FileType		ldx	#NULL
			stx	NameOfClass
;			beq	doSelectFile

;*** Datei auswählen:
;    Übergabe: AKKU = Dateityp (APPL_DATA)
:doSelectFile		jsr	DoFileBox		;Datei auswählen.
			bcc	:52			; => OK, weiter...
::51			sec
			rts
...
;*** Tastaturabfrage.
:SelectTaskKeyB		ldx	regMenuCurrent		;Register "Anwendungen" aktiv?
			dex
			bne	exitSlctTask		; => Nein, Ende...

			lda	keyData			;Taste gedrückt?
			beq	exitSlctTask		; => Nein, Ende...
			cmp	#17			;"KEY_DOWN" = Cursor down?
			beq	:next			; => Ja, nächster Task.
			cmp	#16			;"KEY_UP" = Cursor up?
			beq	:last			; => Ja, vorheriger Task.
			cmp	#13			;"KEC_CR" = RETURN?
			bne	exitSlctTask		; => Nein, Ende...

			jmp	OpenSlctTask		;Gewählten Task öffnen.

::next			ldx	SelectedTask
			inx
			cpx	MaxTaskInstalled	;Ende erreicht?
			beq	exitSlctTask		; => Ja, Ende...
			txa
			jmp	PrintNewTask		;Neuen Task auswählen.

::last			ldx	SelectedTask		;Bereits am Anfang?
			beq	exitSlctTask		; => Ja, Ende...
			dex
			txa
			jmp	PrintNewTask		;Neuen Task auswählen.
...
:SetNewTaskName		jsr	GetFreeBank		;Freie Bank suchen.
...
::52			cmp	CurDkDrvConfig -8,x
			...
			cpx	#12
			bcc	:51
::53			ldx	#$00			;$00 = NO_ERROR.
			rts

::54			ldx	#< Dlg_ConfigErr	;Konfiguration geändert.
			ldy	#> Dlg_ConfigErr	;Dialogbox ausgeben und Ende...
			jsr	DoSysDlgBox
			ldx	#$0d			;$0D = DRIVE_NOT_FOUND.
			rts
...
:OPEN_AutoExec		lda	#AUTO_EXEC
			...
			jsr	Slct_ApplExec		;Datei auswählen.
			bcs	:51			; => Abbruch...
...
;Hilfsmittels überschreiben.
			...
			lda	#DESK_ACC
			jsr	Slct_FileType		;Hilfsmittel auswählen.
...
:OPEN_ApplData		lda	#APPL_DATA
			jsr	Slct_ApplData		;Datei auswählen.
			bcc	:52			; => Abbruch...
::51			rts
...
::54			lda	fileHeader +$75,y
			...
			txa				;Gefunden ?
			beq	:appl_ok		; => Ja, weiter...

			ldx	#< Dlg_ApplNFnd		;Fehler: Anwendung nicht gefunden.
			ldy	#> Dlg_ApplNFnd		;Dialogbox ausgeben und Ende...
			jmp	DoSysDlgBox

::appl_ok		ldy	#$0f			;Disketten-/Dateiname für
...
:IsApplOnDsk		lda	#%10000000		;Applikation auf RAM-
			...
			jsr	FindAppl		;Laufwerken suchen...
			bcc	:51
			ldx	#$05			;$05 = FILE_NOT_FOUND.
			rts
::51			ldx	#$00			;$00 = NO_ERROR.
			rts
...
:ScreenShot		php
			...
			lda	CurTaskBank		;Bank-Adresse setzen.
			sta	r3L
...
:MenuText00		b PLAINTEXT
			...
			b $06
if LANG = LANG_DE
			b "GDOS64 - Anwendungen verwalten",NULL
endif
if LANG = LANG_EN
			b "GDOS64 - Application manager",NULL
endif
...
;*** Dialogbox: Applikation nicht gefunden.
:Dlg_ApplNFnd		b %01100001
			b $30,$97
			w $0040,$00ff
...
::51			b PLAINTEXT,BOLDON
			b "FEHLER!",PLAINTEXT,NULL
::52			b "Die erforderliche GEOS-Anwendung",NULL
::53			b "für das Dokument konnte auf keinem",NULL
::54			b "Laufwerk gefunden werden!",NULL
endif
if LANG = LANG_EN
::51			b PLAINTEXT,BOLDON
			b "ERROR!",PLAINTEXT,NULL
::52			b "The required GEOS application for",NULL
::53			b "the document could not be found",NULL
::54			b "on any drive!",NULL
endif

;*** Dialogbox: Konfiguration wurde geändert.
:Dlg_ConfigErr		b %01100001
			b $30,$97
			w $0040,$00ff
...
::51			b PLAINTEXT,BOLDON
			b "FEHLER!",PLAINTEXT,NULL
::52			b "Laufwerkskonfiguration wurde geändert!",NULL
::53			b "Bitte vor dem wechseln zu einer",NULL
::54			b "anderen Anwendung die Konfiguration",NULL
::55			b "der Laufwerke zurücksetzen!",NULL
endif
if LANG = LANG_EN
::51			b PLAINTEXT,BOLDON
			b "ERROR!",PLAINTEXT,NULL
::52			b "Drive configuration has been changed!",NULL
::53			b "Please reset the configuration of the",NULL
::54			b "drives before switching to another",NULL
::55			b "application!",NULL
endif
...
:COLOR1			b USE_COLOR_INPUT
:RegTIcon1_1_09		w Icon_OPEN
			b $17,$a0,$06,$10
:COLOR2			b USE_COLOR_INPUT
...
:RegTIcon1_4_01		w Icon_OK
			b $1d,$48,$06     ,$10
:COLOR4			b USE_COLOR_INPUT
:RegTIcon1_4_03		w Icon_DRIVE_A
			b $1b,$78,$02     ,$10
:COLOR5			b USE_COLOR_INPUT
:RegTIcon1_4_04		w Icon_DRIVE_B
			b $1d,$78,$02     ,$10
:COLOR6			b USE_COLOR_INPUT
:RegTIcon1_4_05		w Icon_DRIVE_C
			b $1f,$78,$02     ,$10
:COLOR7			b USE_COLOR_INPUT
:RegTIcon1_4_06		w Icon_DRIVE_D
			b $21,$78,$02     ,$10
:COLOR8			b USE_COLOR_INPUT
