...
::1			jsr	PosToNextCard		;Zeiger auf nähstes CARD.
			beq	:3			;Fertig ? Ja, weiter...

			lda	(r6L),y			;Byte aus Speicher #1 in
			sta	(r5L),y			;Speicher #2 kopieren.
			jmp	:1			;Nächstes Byte kopieren.

;*** Nur 1 CARD beschreiben.
::2			lda	r8L			;Bits im ersten und letzten
			ora	r8H			;CARD addieren.
			jmp	:4

::3			lda	r8H			;Linienmuster für letztes CARD.
::4			jsr	LinkGrafxMem		;Muster berechnen.
...
::1			stx	r7L			;Aktuelle Zeile merken.
...
::2			lda	r7H			;Bit an X-Koordinate setzen.
			ora	(r5L),y
::3			sta	(r6L),y
			sta	(r5L),y
			ldx	r7L
...
::1			jsr	$ffff			;Rechteck-Job ausführen.
			lda	r11L
			inc	r11L
