...
:AnalyzeNxFile		jsr	TestExitKey		;Setup beenden?

			lda	EntryPosInArchiv	;Zeiger auf aktuelle Datei.
			asl
			tay
			lda	r1L			;Startadresse des ersten Sektors
			sta	PackFileSek +0,y	;zwischenspeichern.
			lda	r1H
			sta	PackFileSek +1,y

			ldy	EntryPosInArchiv
			lda	Vec2SourceByte		;Zeiger auf Byte innerhalb Sektor
			sta	PackFileByt,y		;zwischenspeichern.
...
if LANG = LANG_EN
			b "Please be patient for a short moment while the"
endif
			b GOTOXY
			w $0010
			b $60
if LANG = LANG_DE
			b "während das SETUP-Programm das Archiv mit den"
endif
if LANG = LANG_EN
			b "SETUP program examines the archive with packed"
endif
			b GOTOXY
			w $0010
			b $68
if LANG = LANG_DE
			b "gepackten GDOS64 Systemdateien untersucht."
endif
if LANG = LANG_EN
			b "GDOS64 system files."
endif
			...
			b $78
if LANG = LANG_DE
			b "Dieser Vorgang kann mehrere Minuten dauern..."
endif
if LANG = LANG_EN
			...
			b $84
if LANG = LANG_DE
			b "Um diesen Vorgang zu beschleunigen, starten Sie"
endif
if LANG = LANG_EN
			b "To speed up this process, start the SETUP program"
endif
			b GOTOXY
			w $0010
			b $8c
if LANG = LANG_DE
			b "SETUP von einem RAM-Laufwerk (RAM81/RAMNative)!"
endif
if LANG = LANG_EN
			b "from a RAM drive (RAM81/RAMNative)!"
endif
