...
:MainInit		sei
			...
			lda	curDirHead +0		;Zeiger auf ersten Sektor des
			sta	curDirTrack		;Hauptverzeichnisses einlesen.
			lda	curDirHead +1
			sta	curDirSektor
			lda	#$00			;Zeiger auf ersten Verzeichnis-
			sta	Vec2DirEntry		;Eintrag setzen.

;--- Zuerst GD.CONFIG ausführen.
			LoadW	r6,FNamGConf
			jsr	FindFile		;GD.CONFIG suchen.
			txa				;Diskettenfehler?
			bne	ExitAutoBoot		; => Ja, Abbruch...

::doGDConfig		lda	#> :doAutoExec		;EnterDeskTop-Vektor auf
			sta	EnterDeskTop +2		;AutoBoot-Routine umlenken.
			lda	#< :doAutoExec
			sta	EnterDeskTop +1

			jmp	StartDirEntry		;GD.CONFIG starten.

;--- Danach alle AutoExec ausführen.
::doAutoExec		MoveB	curDirTrack ,r1L	;Zeiger auf aktuellen Verzeichnis-
			MoveB	curDirSektor,r1H	;Sektor setzen.

			jmp	TestNextEntry		;AutoExec-Programme starten.
...
;*** Aktuelle Datei laden und starten.
:StartDirEntry		LoadB	r0L,$00
