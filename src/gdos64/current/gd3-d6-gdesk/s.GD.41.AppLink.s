...
:xLNK_LOAD_DATA		jsr	TempBootDrive		;AppLink.cfg-Laufwerk aktivieren.
			txa				;Laufwerksfehler?
			bne	:1			; => Ja, Abbruch...

			jsr	linkFindConfig		;Konfigurationsdatei suchen.
			txa				;Datei gefunden?
			beq	:2			; => Ja, weiter...
::1			rts				; => Abbruch...

::2			jsr	linkGetVHeader		;VLIR-Header einlesen.
			txa				;Fehler?
			bne	:1			; => Ja, Abbruch...

			jsr	linkInitConfig		;VLIR-Daten initialisieren.
...
;			jsr	OpenBootDrive		;AppLink.cfg-Laufwerk öffnen.
;			txa				;Fehler?
;			bne	:1			; => Ja, Abbruch...
...
::3			lda	ND_Record		;Zeiger auf AppLink einlesen.
			...
			jsr	linkSelectPage		;Zeiger auf VLIR-Datensatz setzen.
			...
			jsr	linkNextPage		;Zeiger auf nächsten AppLink.
...
:linkFindConfig		LoadB	r7L,SYSTEM		;Typ: Systemdatei.
			LoadB	r7H,1			;Nur erste Datei finden.
			LoadW	r6,tempALnkNam		;Dateiname.
			LoadW	r10,GD_APPL_CLASS	;GEOS-Klasse.
			...
			LoadW	r6,tempALnkNam		;Datei suchen um ":dirEntryBuf"
...
:linkGetVHeader		jsr	linkSetVHeader		;VLIR-Header-Daten setzen.
			jmp	GetBlock

;*** VLIR-Daten initialisieren.
:linkInitConfig		LoadB	ND_Record,0		;Zeiger auf ersten Datensatz.
			LoadW	r14,LinkData +LINK_DATA_BUFSIZE +1
			LoadW	r15,appLinkIBufU
			rts

;*** Zeiger auf nächsten AppLink-Datenpuffer.
:linkNextPage		AddVBW	LINK_DATA_BUFSIZE,r14
			AddVBW	64,r15
			rts
...
:linkSelectPage		asl				;Zeiger auf VLIR-Sektor einlesen.
			tay
			ldx	ND_VLIR +2,y
			stx	r1L
			lda	ND_VLIR +3,y
...
:linkSetVHeader		lda	dirEntryBuf +1		;VLIR-Header einlesen.
			sta	r1L
			lda	dirEntryBuf +2
			sta	r1H
			LoadW	r4,ND_VLIR
...
::link_unknown		lda	#< Icon_DEL		; => System-Icon: Unknown.
			ldx	#> Icon_DEL
...
::loop1			lda	(r0L),y			;Icon-Daten in Link-Daten
			...
			jmp	OpenBootDrive		;AppLink.cfg-Laufwerk öffnen.
...
:xLNK_SAVE_DATA		jsr	TempBootDrive		;AppLink-Konfig-Laufwerk öffnen.
			txa				;Laufwerksfehler?
			bne	:err_linkdrv		; => Ja, Abbruch...

			jsr	linkFindConfig		;AppLink-Datei suchen.
			txa				;Fehler?
			beq	:init_save		; => Nein, weiter...

			jsr	linkNewConfig		;Neue Konfigurationsdatei schreiben.
			txa				;Fehler?
			beq	:init_save		; => Nein, weiter...
...
;--- Initialisierung.
::init_save		jsr	linkGetVHeader		;VLIR-Header einlesen.
			txa				;Fehler?
...
;			sta	ND_Data +0		;Ende Datensatz Track=$00.
			...
			jsr	linkDeletePage		;Sektor freigeben.
			txa				;Fehler?
...
::save_data		lda	ND_Record		;Datensatz-Nr. einlesen.
			...
			jsr	linkCreatePage		;Freien Sektor suchen.
			txa				;Fehler?
			bne	:err_savedata		; => Ja, Abbruch...
...
::close_config		jsr	linkSetVHeader		;VLIR-Header-Daten setzen.
...
:linkNewConfig		jsr	OpenDisk		;Diskette öffnen.
			txa				;Fehler?
			beq	:2			; => Nein, weiter...
::1			rts				;Abbruch.
...
::rename		LoadW	a0,tempALnkNam		;Zeiger auf Zwischenspeicher.
...
:HdrB117		t "opt.GDesk.Build"  		;Anwendung/Klasse/Version/NULL
...
:FNameALConf		b "GeoDesk.lnk",NULL
:tempALnkNam		s 17
