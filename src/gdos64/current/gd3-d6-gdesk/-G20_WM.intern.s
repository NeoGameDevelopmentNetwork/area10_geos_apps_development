...
;
:WM_CALL_RIGHTCLK	jsr	waitNoMseKey		;Warten bis keine M-Taste gedrückt.
...
:scrnSizeData		b $00				;Bildschirm/Desktop.
			b SCRN_HEIGHT -1
			w $0000
			w SCRN_WIDTH -1

;
;Routine  : WM_RESET_AREA
;Parameter: -
;Rückgabe : -
;Verändert: A
;Funktion : Grenzen für Textausgabe/Mauszeiger zurücksetzen.
;
:WM_RESET_AREA		txa
			pha

			ldx	#5
::1			lda	scrnSizeData,x
			sta	windowTop,x
			sta	mouseTop,x
			dex
			bpl	:1
...
:WM_DRAW_SLCT_WIN	jsr	WM_GET_SLCT_SIZE	;Fenstergröße einlesen.
			...
			jsr	WM_RESET_AREA		;Rand für Textausgabe zurücksetzen.

;--- System-Icons zeichnen.
			lda	#< :tab1		;Zeiger auf Icon-Tabelle.
			sta	r14L
			lda	#> :tab1
			sta	r14H
			lda	#5			;5 Icons...
			sta	r15H
			lda	C_WinTitel		;Farbe für Titel-/Statuszeile.
			sta	r13H
...
;--- Resize-Icons zeichnen.
			...
			lda	#< :tab2		;Zeiger auf Icon-Tabelle.
			sta	r14L
			lda	#> :tab2
			sta	r14H
			lda	#4			;4 Icons...
...
::1			lda	WM_DATA_MOVEBAR		;Scrollbalken anzeigen?
			...
			lda	#< :tab3		;Zeiger auf Icon-Tabelle.
			sta	r14L
			lda	#> :tab3
			sta	r14H
			lda	#2			;2 Icons...
...
;
:WM_DRAW_USER_WIN	lda	r2L			;Y-Oben speichern.
			pha
			lda	r3H			;X-Links speichern.
			pha
			lda	r3L
			pha
...
;--- Fußzeile zeichnen.
			...
			lda	r2H			;Ende des Fensters mit einer Linie
			sta	r11L			;markieren, damit Statuszeile und
			lda	#%10101010		;Kopfzeile verschiedener Fenster
			jsr	HorizontalLine		;unterscheidbar sind.
...
:WM_DRAW_ICON_TAB	ldy	#$03			;Routine zum definieren des
			...
			lda	(r14L),y
			sta	r0H
			lda	#Icon_MoveW		;Icon-Breite.
			sta	r2L
			lda	#Icon_MoveH		;Icon-Höhe.
			sta	r2H
			jsr	BitmapUp		;Bitmap anzeigen.

			lda	r14L			;Zeiger nächstes Icon in Tabelle.
			clc
			adc	#4
			sta	r14L
			bcc	:1
...
::1			dec	r15H			;Alle Icons angezeigt?
...
;
:WM_DRAW_TITLE		jsr	WM_RESET_AREA
...
;
:WM_DRAW_INFO		jsr	WM_RESET_AREA
			jsr	WM_GET_SLCT_SIZE
...
;--- Anzahl Einträge je Seite.
			...
			lda	#< :tmp_MovData		;Zeiger auf Datentabelle.
			sta	r0L
			lda	#> :tmp_MovData
			sta	r0H
...
:WM_DRAW_FRAME		lda	r2L			;Y-Oben speichern.
			...
			lda	r3L			;Linken Rand invertieren.
			sta	r4L
			lda	r3H
			sta	r4H
			jsr	InvertRectangle
...
;			lda	r4L			;Rechten Rand invertieren.
;			sta	r3L
;			lda	r4H
;			sta	r3H
			jsr	InvertRectangle
...
;
:WM_TEST_WIN_POS	lda	WM_DATA_CURENTRY
			sta	r0L
			jsr	WM_TEST_CUR_POS
...
::1			;beq	:4			;Erster Eintrag auf Seite gültig?
			bcc	:4			; => Ja, Ende...

::2			lda	WM_DATA_MAXENTRY
			sec
...
;möglich. Verschiebung nicht nötig.
			...
			lda	WM_DATA_Y0		;Fenster-Position wieder auf
			sec				;Y-Position zurücksetzen.
			sbc	#8*3
			sta	WM_DATA_Y0
...
;			lda	WM_DATA_X0 +0		;X-Position nicht verändern.
;			clc
;			adc	#8
;			sta	WM_DATA_X0 +0
;			bcc	:add3
...
::2			lda	WM_DATA_Y0		;Fenster um 1 CARD nach unten
			clc				;verschieben.
			adc	#8
			sta	WM_DATA_Y0
			...
			lda	WM_DATA_X0 +0		;Fenster um 2 CARDs nach rechts
			clc				;verschieben.
			adc	#16
			sta	WM_DATA_X0 +0
			bcc	:add1
...
;Parameter: WM_WCODE = Fenster-Nr.
;           WM_DATA_BUF = Daten für aktuelles Fenster.
;           WM_DATA_WINMOVE = Zeiger auf Routine für Scrolling.
;Rückgabe : -
;Verändert: A,X,Y,r0-r15
;Funktion : Fensterinhalt verschieben.
;           $0000 = Nichts verschieben.
;           $FFFF = Standard-Routine für Icons.
;           $EEEE = Standard-Routine für Text.
;
...
;--- Nach oben verschieben.
::moveLineUp		lda	WM_DATA_CURENTRY
			beq	:exit
			sec
			sbc	WM_COUNT_ICON_X
			sta	WM_DATA_CURENTRY
...
::32			lda	r2L			;Grafik der letzte Zeile löschen.
			sta	r0L
			lda	r2H
			sta	r0H
			jsr	ClearRam
...
;--- Neuen Eintrag in letzter Zeile ausgeben.
			...
			lda	r5L			;Erster Eintrag in letzter Zeile.
			sta	r0L
...
::42			lda	r2L			;Grafik der ersten Zeile löschen.
			sta	r0L
			lda	r2H
			sta	r0H
			jsr	ClearRam
...
::21			jsr	:moveToFile		;Position in Dateiliste berechnen.
			...
			lda	#%0000000
			sta	pressFlag		;Maustastenklick löschen.
...
;--- Eine Seite vorwärts.
::moveNextPage		lda	WM_DATA_CURENTRY
			clc
			adc	WM_COUNT_ICON_XY
			sta	r0L
...
;--- Eine Seite zurück.
::moveLastPage		lda	WM_DATA_CURENTRY
			sec
			sbc	WM_COUNT_ICON_XY
			sta	r0L
			bcs	:31
...
::3			sta	r1L
			...
			inc	CurEntry		;Zeiger auf nächsten Eintrag.

::4			lda	CurEntry		;Alle Daten ausgegeben?
			cmp	WM_DATA_MAXENTRY
::4a			bcs	:6			; => Ja, Ende...
...
::3			jsr	setEntryData		;Daten für aktuellen Eintrag setzen.
			...
			inc	CurEntry		;Zähler auf nächsten Eintrag.
