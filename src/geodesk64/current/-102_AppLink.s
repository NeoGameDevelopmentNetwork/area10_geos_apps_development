...
:AL_DRAW_FILES		jsr	ResetFontGD		;GeoDesk-Font aktivieren.
...
::0			stx	:1 +1			;Zeiger auf AppLink-Icon-Daten.
			sty	:1 +2

			ldy	#$3f			;63 Bytes an Icon-Daten in
::1			lda	$ffff,y			;AppLink-Daten kopieren.
			sta	(r1L),y
			dey
			bne	:1
...
;    Übergabe: AKKU/XREG = Zeiger auf Farbtabelle.
...
::2			sta	(r0L),y			;kopieren: 3x3 Bytes.
			inx
			iny
			cpx	#$09
			bcc	:2
...
;              AL_TYPE_PRNT   : Drucker.
;              AL_TYPE_DRIVE  : Laufwerk.
...
;*** Fensteroptionen:
;Die Optionen werden dabei als BITs
;innerhalb eines Options-Byte in den
;AppLink-Daten gespeichert.
...
:setALDat_WMode		lda	#$00			;Fensteroptionen in AppLink
...
;--- Hinweis:
;Hier muss das oberste Dateifenster
;aktualisiert werden, da sonst durch
;das laden des Druckertreibers die
;interne Verzeichnisliste teilweise
;überschrieben wurde.
			jmp	MOD_UPDATE		;Zurück zum Hauptmenü, oberstes
...
::err			jsr	SUB_OPENPRNT_ERR	;Druckertreiber-Fehler ausgeben.
			jmp	UpdateMyComputer	;"MyComputer"-Fenster aktualisieren.

;*** Eingabegerät wechseln.
:AL_OPEN_INPUT		jsr	SUB_SLCT_INPT		;Eingabetreiber wechseln.
			txa				;Abruch?
			bne	:1			; => Ja, Ende...
			jmp	UpdateMyComputer	;"MyComputer"-Fenster aktualisieren.
::1			rts
...
::3			ldx	curDrive
			...
			lda	diskBlkBuf,x
			and	#FTYPE_MODES
			cmp	#FTYPE_DIR		;Ist Eintrag noch ein Verzeichnis?
...
:AL_OPEN_NEW_DSK	lda	#$00			;Partition-/DiskImage-Modus löschen.
			b $2c
