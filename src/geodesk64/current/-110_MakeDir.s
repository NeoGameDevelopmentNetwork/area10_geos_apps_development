...
:MakeNDir		jsr	OpenDisk		;Disk öffnen/BAM einlesen.
			...
			LoadB	r3L,1			;Freien Sektor für neuen
...
::101			lda	r3L
			ldx	r3H
::102			sta	NewDirHd+0		;1.Reservierten Sektor merken.
			stx	NewDirHd+1

			jsr	SetNextFree		;Sektor für Verzeichnis suchen.
			txa				;Fehler?
			bne	:100			; => Ja, Abbruch...
...
; >> cannot be found, no directory will be created.
			lda	r3L			;Prüfen ob 2.Sektor neben dem
			ldx	r3H			;ersten Sektor angeordnet ist.
			cmp	NewDirHd +0
			bne	:103
			cpx	#$00
			beq	:103
...
::103			lda	NewDirHd +0		;Sektor für Verzeichnis-Header
			...
			cmp	#$00			;verwenden.
			bne	:102			; -> Track gültig.

			ldx	#INV_TRACK
			bne	:100			; -> Track ungültig, Fehler.

::110			inx
			sta	NewDirSk +0		;2.Reservierten Sektor merken.
			stx	NewDirSk +1
