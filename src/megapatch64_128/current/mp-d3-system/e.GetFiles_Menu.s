...
;*** Icon-Menü ausgeben.
:yGetFiles_Menu		jsr	DrawDBoxWithICol
...
::51			lda	DB_Icon_Tab		;Icons definiert ?
			beq	:52			; => Nein, weiter...
			jsr	DrawDBIconCol		;Icons ausgeben.

::52			jmp	Exit_DrawIcons		;Zurück zum Kernal.

;*** Icons für GetFile-Menu ausgeben.
:DrawDBoxWithICol	PushB	Flag_DBoxType		;Dialogbox-Kennbyte speichern.
			...
			PopB	Flag_DBoxType		;Dialogbox-Kennbyte zurücksetzen.
:NOFUNC0		rts

;*** Icon-Farbe setzen wenn Flags gesetzt.
:DrawDBIconCol		lda	DB_Icon_Tab
			beq	NOFUNC0
...
if Flag64_128 = TRUE_C128
			and	#%01111111		;Double-Bit ausblenden für Abfrage
endif
...
::51			ldx	DB_Icon_Tab		;Anzahl vorhandener Icons einlesen.
			...
			lda	C_DBoxDIcon		;Vorgabe für Dialogbox-Farbe.
			bit	Flag_GetFiles		;Dateiauswahlbox?
			bpl	:54			; => Nein, weiter..
			lda	C_FBoxDIcon		;Vorgabe für Auswahlbox-Farbe.
::54			sta	DB_IconColor,x		;Icon-Farbe festlegen.
			...
			jsr	DB_CopyIconInTab	;Icon in Tabelle kopieren (Kernel).
...
if Flag64_128 = TRUE_C128
			ora	#%10000000		;Double-Bit einblenden für Subtraktion
endif
			sec
			sbc	DB_Icon_Tab + 8,y
...
if Flag64_128 = TRUE_C128
			ora	#%10000000		;Double-Bit einblenden für Speicherung
endif
			sta	DB_Icon_Tab + 6,y
			lda	r0H
			sta	DB_Icon_Tab + 7,y
...
;Bit%5 zeigt aktive Dialogbox an.
;Siehe Kernal-DB_DrawBox.
			lda	#ST_WR_FORE ! %00100000
			sta	dispBufferOn
			...
			w	(X_DBox+ 8) ! DOUBLE_W
			w	(X_DBox + B_DBox + 7) ! DOUBLE_W ! ADD1_W
...
::2
			...
			LoadW	r3,(X_DBox + 8) ! DOUBLE_W
			LoadW	r4,(X_DBox + B_DBox - 8) ! DOUBLE_W
...
:DrvIconTab		w Icon_DRIVE_A
			b $00 ! DOUBLE_B,$00,$02 ! DOUBLE_B,$10
			w DB_Icon_DrvA
