...
.NO_OPT_UPDATE		= NULL				;Bei Klick keine Option aktualisieren.
...
;--- Ergänzung: 20.02.21/M.Kanet
;Erweiterung von GeoDOS 64 V3:
;Bei RegisterUpdate den Optionsrahmen
;nicht automatisch neuzeichnen.
:Flag_DrawFrame		b $00
...
::1			inc	Flag_DrawFrame

;--- Ergänzung: 20.01.23/M.Kanet
;Unter GEOS128 muss die Koordinate in
;r4 an den 40Z/80Z-Bildschirmmodus
;angepasst werden, da sonst bei der
;Ausgabe von rechtsbündigen Zahlen die
;x-Koordinate falsch berechnet wird.
if Flag64_128 = TRUE_C128
			ldx	#r4L			;r4 wird für die Ausgabe von
			jsr	NormalizeX		;rechtsbündigen Zahlen benötigt.
endif
...
;--- Ergänzung: 20.02.21/M.Kanet
;Erweiterung von GeoDOS 64 V3:
;FarbCode $FF = Farbe Eingabefeld.
;FarbCode $EE = Farbe Registerkarte.
			lda	(r10L),y		;Farbcode einlesen.
			cmp	#USE_COLOR_INPUT	;IconFarbe = Farbe Eingabefeld?
			bne	:1			; => Nein, weiter...
			lda	C_InputField		;z.B. für SelectIcons neben
			jmp	:2			;Auswahlfenster,

::1			cmp	#USE_COLOR_REG		;Icon-Farbe = Registerfarbe?
			bne	:2			; => Nein, weiter...
			lda	C_RegisterBack		;z.B. für Icons auf Registerkarte.
...
if Flag64_128 = TRUE_C128
			lda	r2L
			and	#%01111111		;Double_Bit ausblenden
			sta	r2L
endif
			lda	r1L
if Flag64_128 = TRUE_C128
...
:RegUpdt_SlctOpt	jsr	RegisterNewField	;Optionsfeld zeichnen.
			...
			lda	#RegIcon01_x
if Flag64_128 = TRUE_C128
			bit	r1L			;Double-Bit der X-Koordinate
			bpl	:1			;testen und ggf. Icon-Breite
			ora	#%10000000		;anpassen.
::1
endif
			sta	r2L			;Breite für Options-Icon setzen.

			lda	#RegIcon01_y
			sta	r2H			;Höhe für Options-Icon setzen.
