; UTF-8 Byte Order Mark (BOM), do not remove!
;
; Area6510 (c) by Markus Kanet
;
; This file is used to document the source code, not for
; creating an executable program.
;

if .p
			t "TopSym"
;			t "TopMac"
;			t "Sym128.erg"
			t "ext.BuildMod.ext"
endif

			n "geoUFileSeek"
			c "geoUFileSeekV0.1"
			a "Markus Kanet"

if BUILD_DEBUG = TRUE
			h "/Usb0/ULIB/test.d64"
else
			h "/root/dir/disk.d64"
endif

			h "Daten aus DiskImage auslesen..."

			o APP_RAM
			p MAININIT

			f APPLICATION
			z $40 ;GEOS 40/80-Zeichen.

			i
<MISSING_IMAGE_DATA>

;--- Sprungtabelle.
:MAININIT		jmp	execFileSeek

;--- geoULib einbinden.
;Systemroutinen:
			t "ulib.C.SymbTab"		;Erweiterte Symboltabelle.
			t "ulib.C.Core"			;UCI-Systemroutinen.

;Optionale Routinen:
			t "ulib._ErrUDev"		;Fehler "Kein Ultimate" ausgeben.
			t "ulib._GetStatus"		;UCI-Statusregister abfragen.
			t "ulib._GetData"		;Daten über UCI einlesen.
			t "ulib._AccData"		;Datenempfang bestätigen.
			t "ulib._Push_DWORD"		;DWORD an UCI senden.
			t "ulib._Push_NAME6"		;Dateiname/Pfad in r6 an UCI senden.
			t "ulib._SplitPath"		;/Pfad/Dateiname aufteilen.

;DOS-Routinen:
			t "_dos.00.Target"		;DOS-Target 1/2 setzen.
			t "_dos.02.FOpen"		;Datei öffnen.
			t "_dos.03.FClose"		;Datei schießen.
			t "_dos.04.DRead"		;Daten lesen.
			t "_dos.06.FileSeek"		;Position innerhalb Datei setzen.
			t "_dos.11.ChDir"		;Verzeichnis wechseln.

;Erweiterte Programmroutinen:
			t "inc.Conf.FName"		;Dateiname mit/ohne Pfad.

;*** Daten aus DiskImage auslesen.
:execFileSeek		jsr	ULIB_TEST_UDEV		;Ultimate testen.
;			txa				;Gerät vorhanden?
			beq	:ok			; => Ja, weiter...

;--- Kein Ultimate, Fehler, Ende...
			jsr	ULIB_ERR_NO_UDEV
			jmp	exitDeskTop		; => Ende...

;--- Ultimate vorhanden.
::ok			jsr	getFileName		;Konfiguration einlesen.

			lda	uFileName		;Pfad vorhanden?
			beq	exitDeskTop		; => Nein, Ende...

			jsr	uFileSeek		;Daten aus DiskImage einlesen.
			txa				;Fehler?
			bne	:skip			; => Ja, Kein Diskname anzeigen.

			jsr	copyDiskName		;Diskname kopieren.

::skip			lda	#< dBoxStatus
			sta	r0L
			lda	#> dBoxStatus
			sta	r0H
			jsr	DoDlgBox		;Status anzeigen.

;*** Zurück zu GEOS.
:exitDeskTop		jmp	EnterDeskTop		;Ende.

;*** Daten aus DiskImage einlesen.
;Übergabe : uFileName = (Verzeichnispfad/)Dateiname
;Rückgabe : X = Fehlerstatus, $00=OK
;           UCI_STATUS_MSG = Status-Meldung
;           UCI_DATA_MSG   = Datenspeicher
;           r6 = Zeiger auf Dateiname innerhalb uFileName
;Verändert: A,X,Y,r0,r1,r4,r5,r6,r9

:uFileSeek		jsr	ULIB_IO_ENABLE		;IRQ sperren, I/O ein.
			jsr	ULIB_SEND_ABORT		;Abbruch senden.

			jsr	_UCID_SET_TARGET1	;Target DOS1 verwenden.

			lda	#< UCI_DATA_MSG		;Zwischenspeicher für Pfad.
			sta	r6L
			lda	#> UCI_DATA_MSG
			sta	r6H
			lda	#< uFileName		;Zeiger auf /Pfad/Dateiname.
			sta	r9L
			lda	#> uFileName
			sta	r9H
			jsr	ULIB_SPLIT_PATH		;Pfad+Dateiname aufteilen.
			txa				;Pfad vorhanden?
			beq	:skipcd			; => Nein, weiter...

;			lda	#< UCI_DATA_MSG		;Verzeichnis wechseln.
;			sta	r6L
;			lda	#> UCI_DATA_MSG
;			sta	r6H
			jsr	_UCID_CHANGE_DIR
			txa				;Fehler?
			bne	:err			; => Ja, Abbruch...

::skipcd		lda	r9L			;Zeiger auf Dateiname ohne Pfad.
			sta	r6L
			lda	r9H
			sta	r6H

;			lda	#< uFileName		;Datei zum lesen öffnen.
;			sta	r6L
;			lda	#> uFileName
;			sta	r6H
			jsr	_UCID_OPEN_FILE_READ
			txa				;Fehler?
			bne	:err			; => Ja, Abbruch...

			lda	#< $6590		;Diskname bei D64 ab
			sta	r0L			;Offset $0001:6590.
			lda	#> $6590
			sta	r0H
			lda	#< $0001
			sta	r1L
			lda	#> $0001
			sta	r1H

			jsr	_UCID_FILE_SEEK		;Position in Datei setzen.

			lda	#< 24			;Diskname aus DiskImage
			sta	r0L			;einlesen (24 Bytes).
			lda	#> 24
			sta	r0H

			lda	#< UCI_DATA_MSG
			sta	r4L
			lda	#> UCI_DATA_MSG
			sta	r4H

			jsr	_UCID_READ_DATA		;Vorbereitung für Daten lesen.
			txa				;Fehler?
			bne	:done			; => Ja, Abbruch...

			lda	#< UCI_DATA_MSG
			sta	r4L
			lda	#> UCI_DATA_MSG
			sta	r4H

			jsr	_UCID_READ_NEXT		;Daten aus Datei einlesen.
;			txa				;Fehler?
;			bne	:done			; => Ja, Abbruch...

::done			jsr	_UCID_CLOSE_FILE	;Datei schliesen.

::err			jmp	ULIB_IO_DISABLE		;I/O aus, IRQ freigeben.

;*** Diskname übertragen.
:copyDiskName		ldx	#0			;Diskname konvertieren.
			ldy	#0
::1			lda	UCI_DATA_MSG,x
			beq	:9
			cmp	#$20
			bcc	:2
			cmp	#$7f
			bcc	:3
			cmp	#$a0
			bne	:2
			lda	#" "
			bne	:3
::2			lda	#"."
::3			sta	diskName,y
			iny
			inx
			cpx	#24 -1
			bcc	:1

			lda	#NULL			;Ende Diskname.
::9			sta	diskName,y

			rts

;*** Dialogbox: Status anzeigen.
:dBoxStatus		b %10000001

			b DBTXTSTR,$10,$10
			w :info
			b DBTXTSTR,$10,$20
			w uFileName

			b DBTXTSTR,$10,$2a
			w :disk

			b DBTXTSTR,$10,$40
			w UCI_STATUS_MSG

			b OK         ,$10,$48
			b NULL

::info			b PLAINTEXT,BOLDON
			b "INFORMATION:"
			b PLAINTEXT,NULL

::disk			b "Disk: "
:diskName		s 24
