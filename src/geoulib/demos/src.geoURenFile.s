...
if BUILD_DEBUG = TRUE
			...
			h "Datei umbenennen oder verschieben..."
...
;*** Datei umbenennen.
:execRenFile		jsr	ULIB_TEST_UDEV		;Ultimate testen.
;			txa				;Gerät vorhanden?
			beq	:ok			; => Ja, weiter...
...
::ok			jsr	getConfig		;Konfiguration einlesen.
			...
			lda	uOldFile		;Alter/Neuer Name vorhanden?
			beq	exitDeskTop
			lda	uNewFile
			beq	exitDeskTop		; => Nein, Ende...

			jsr	uRenameFile		;Datei umbenennen.
...
;*** Datei umbenennen.
;Übergabe : uOldFile = Alter Dateiname
;           uNewFile = Neuer Dateiname
...
:uRenameFile		jsr	ULIB_IO_ENABLE		;IRQ sperren, I/O ein.
...
;			bne	:err			; => Ja, Abbruch...
			...
			lda	#< uOldFile		;Zeiger auf alten Dateinamen.
			sta	r6L
			lda	#> uOldFile
			sta	r6H
			lda	#< uNewFile		;Zeiger auf neuen Dateinamen.
			sta	r8L
			lda	#> uNewFile
			sta	r8H
			jsr	_UCID_RENAME_FILE
...
;Rückgabe: uOldFile    = Alter Dateiname
...
;*** Dateinamen für Rename.
:uOldFile		s 96
:uNewFile		s 96

;*** Dialogbox: Status anzeigen.
:dBoxRename		b %10000001
