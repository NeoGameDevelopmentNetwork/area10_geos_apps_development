...
if BUILD_DEBUG = TRUE
			h "09:B:/Usb0/ULIB/test.d64"
else
			h "00:X:/root/dir/disk.ext"
endif

			h "Mount Disk in Ultimate-Laufwerk..."
...
:execDiskMnt		jsr	ULIB_TEST_UDEV		;Ultimate testen.
;			txa				;Gerät vorhanden?
			beq	:ok			; => Ja, weiter...
...
;Verändert: A,X,Y,r5,r6,r7L,r7H,r9

:uDiskMount		jsr	ULIB_IO_ENABLE		;IRQ sperren, I/O ein.
...
::skipcd		lda	r9L			;Zeiger Dateiname ohne Pfad.
			sta	r6L
			lda	r9H
			sta	r6H
...
;			lda	geosDriveAdr		;GEOS-Laufwerksadresse.
;			sta	r7L
			jsr	ULIB_DIMG_TYPE		;DiskImage testen.
			txa				;Format gültig?
...
if FALSE
			...
			cpy	#":"
			bne	:exit			; => Ungültig...
			cpy	fileHeader +164		;Trennzeichen?
			bne	:exit			; => Ungültig...

;--- Hinweis:
;Angabe Verzeichnis ist Optional...
;			ldy	fileHeader +165		;Verzeichnispfad?
;			cpy	#"/"
;			bne	:exit			; => Nein, Ungültig...
			...
			lda	fileHeader +163		;GEOS-Laufwerk definiert?
			cmp	#"A"
			bcc	:copy_path		; => Nein, weiter...
			cmp	#"D" +1
			bcs	:copy_path		; => Nein, weiter...
			sec
			sbc	#"A" -8
			sta	geosDriveAdr		;GEOS-Laufwerksadresse.

::copy_path		ldy	#0			;Pfad kopieren.
::loop			lda	fileHeader +160 +5,y
			cmp	#CR
			beq	:done
			sta	uFileName,y
			beq	:done
			iny
			cpy	#96 -5			;Max. Länge -5 (00:A:) erreicht?
