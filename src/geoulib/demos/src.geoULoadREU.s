...
if BUILD_DEBUG = TRUE
			...
			h "DiskImage in Ultimate-RAM-Laufwerk laden..."
...
;*** DiskImage in GEOS-RAMDisk laden.
:execLoadREU		jsr	ULIB_TEST_UDEV		;Ultimate testen.
;			txa				;Gerät vorhanden?
			beq	:ok			; => Ja, weiter...
...
::ok			jsr	getConfigREU		;Konfiguration einlesen.
			...
			jsr	uLoadRAMDisk		;RAMDisk laden.
			txa				;Fehler?
...
::done			bit	firstBoot		;GEOS-Boot?
			bpl	exitDeskTop		; => Ja, Kein Status anzeigen...
...
:uLoadRAMDisk		jsr	ULIB_IO_ENABLE		;IRQ sperren, I/O ein.
...
;			lda	#< uPathRDisk		;Zeiger auf DiskImage.
;			sta	r6L
;			lda	#> uPathRDisk
;			sta	r6H
;			lda	geosDriveAdr		;GEOS-Laufwerksadresse.
...
::size			lda	#$00			;Startadresse löschen.
			...
			ldy	ramBase -8,x		;Startadresse RAMDisk.
			sty	r1L
...
::is1571		pha				;Offset speichern.
			...
			jsr	_UCID_LOAD_REU		;Teil #1 laden (Track 1-35).
			...
			adc	r2H			;Teil #2 laden (Track 36-70).
...
::cont			jsr	_UCID_LOAD_REU		;DiskImage laden.
;			txa				;Fehler?
;			bne	:exit			; => Ja, Abbruch...
